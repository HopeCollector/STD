...
# load params
dataset = dataset_configs("/ws/output/gt/IOU-BBox")
params = []
cnt = 0
for _, seqs in dataset.items():
    for seq in seqs:
        ...
        param["output"] = f"/ws/output/expr/IOU-BBox/{param['id']}.txt"
        params.append(param)
        cnt += 1
...
CMD = 'bash -c "source /ws/install/setup.bash && rosrun std_detector expr -c {0} > /dev/null 2>&1"'
...
id = 0
params[id]["IDX_THD"] = 302
params[id]["IOU_THD"] = 0.5
